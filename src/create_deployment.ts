...
const rawSDL = fs.readFileSync(
  path.resolve(__dirname, "./required-files/example.sdl.yml"),
  "utf8"
);
const certificatePath = path.resolve(__dirname, "./required-files/cert.json");
...
async function walletFromMnemonic(mnemonic: string) {
  try {
    ...
    console.error(
      'Could not create wallet from mnemonic, have you updated ".env" file?'
    );
    throw error;
  }
}
    ...
    if (status) {
      for (const [name, service] of Object.entries(status.services)) {
        if (service.uris) {
          ...
          // NOTE: This is a hack to get the URL of the service written to the chain. Normally, you would post this to a service registry and simply return, which would successfully end the acurast deployment.
          throw new Error(
            `Service ${name} is available at: ${service.uris[0]}`
          );
        }
      }
    }
